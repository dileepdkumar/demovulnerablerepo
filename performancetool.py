...
create_apps=60
fdevsec_dashboard_ip='10.34.160.14'
...
payload = {'username':'dkumar@fortinet.com','password':'Fortinet@2015'}
...
for x in range(create_apps):
    try:
        ...
        params_data  = {'app_name':"apptest"+str(x),'org_id': org_id}
        ...
        container=client.containers.run('qa.fortidevsec.forticloud.com/fortidevsecops/fdevsec_sast:latest',remove=False,extra_hosts={'qa.fortidevsec.forticloud.com':fdevsec_dashboard_ip},volumes={pwd : {'bind': '/scan', 'mode': 'rw'}},detach=True)
        ...
        headers_details={'Authorization': r4_data['token_type'] + ' ' + r4_data['access_token'],'accept':r4.headers['Content-Type']}
        r6 = requests.get('https://qa.fortidevsec.forticloud.com/api/v1/dashboard/get_apps', headers=headers_details, params = {'org_id': org_id}, verify=False)
        #print (r6.json())
        ...
        payload = {'username':'dkumar@fortinet.com','password':'Fortinet@2015'}
        r4 = requests.post('https://qa.fortidevsec.forticloud.com/api/v1/login/access-token', verify=False,data=payload)
        headers_details={'Authorization': r4_data['token_type'] + ' ' + r4_data['access_token'],'accept':r4.headers['Content-Type']}
        continue
    else:
        continue
